...
| [`2026-09-09_PALANTIR_ONTOLOGY_LESSONS.md`](2026-09-09_PALANTIR_ONTOLOGY_LESSONS.md) | Foundry's Ontology read against the live stack: primitive-by-primitive map, the Execution/Result gap, **NSP-13 Governed Actions** (NSP-0 is the manifest), Mecky's three tiers in keys not config, brand rules per surface | Current — v2 of Max's research, 2026-09-09 |
...
| [`2026-07-31_CONDUIT_RAAS_STRATEGY.md`](2026-07-31_CONDUIT_RAAS_STRATEGY.md) | **Conduit.xyz / RaaS business-model study**: verified 2026 snapshot of Conduit (product, $60k/yr + 5% pricing, Ronin migration, marketplace) mapped onto Netizen. Verdict = adopt the business *shape* (declarative config → few-click deploy, base fee + aligned share, self-serve→white-glove funnel, migrations as credential, no token), build **paymaster + TenantFactory/CommunityRegistry + coordinator-as-a-service**, skip sequencers/own chain/RPC business; bring-your-own-rollup posture for the EEZ/Ethereum-settlement future | Current — researched 2026-07-31 |
| [`2026-07-31_MACI_V3_MIGRATION_DECISION.md`](2026-07-31_MACI_V3_MIGRATION_DECISION.md) | **MACI v3 vs v2.5 decision**: stay on v2.5, build the factory against it, migrate only on defined triggers. Key findings: the Shamir federation *survives* v3 (`generateProofs -k`, same `macisk.` key format); v3 is externally audited (HashCloak 2026-03) with a completed trusted setup and pre-deployed Gnosis infra — but is 6 weeks old, unpatched, has zero production users, PSE was disbanded a week after release, and migration = third re-signup + per-poll client-side proving in Expo + Governor/gatekeeper/pipeline rewrite. **Corrects the corpus**: v3 does NOT eliminate deploy-per-vote | Decided — 2026-07-31 |
| [`2026-09-14_LOGOS_STACK_ASSESSMENT.md`](2026-09-14_LOGOS_STACK_ASSESSMENT.md) | **Logos stack (ex-Waku/Codex/Nomos) integration assessment**: four sourced research passes mapped against the live stack. Verdict = integrate nothing in 2026 — Waku has no React Native SDK and Logos Chat is a desktop-only preview; Logos Storage lost its durability engine (no proofs, no marketplace, non-IPFS CIDs); LEZ has no EVM or bridge. The real "CID for every file" is **Blossom + sha256 on the node**; the DM decision stays XMTP-backlog vs Nostr NIP-17. Dated watchlist with triggers | Current — researched 2026-09-14 |
| [`2026-09-15_INTERFOLD_ASSESSMENT.md`](2026-09-15_INTERFOLD_ASSESSMENT.md) | **The Interfold (ex-Enclave) assessment**: Gnosis Guild's FHE + ZK + threshold network on Ethereum mainnet (Network Alpha, Aug 2026), the first production "no coordinator at all" private tally. Verdict = integrate nothing in 2026: browser-only ballots (four UltraHonk proofs, ~4M constraints each, above the team's own browser target), Ethereum-L1-only with per-voter L1 gas, ≥12 h rounds, 16–330 USDS per round. Named as the trigger-based successor of the Shamir coordinator; three honest Kickstarter designs; Expo paths and a dated watchlist | Current — researched 2026-09-15 |
| [`2026-09-25_ORTIS_MULTI_TENANT_PLATFORM_AND_REVENUE.md`](2026-09-25_ORTIS_MULTI_TENANT_PLATFORM_AND_REVENUE.md) | **Ortis as a multi-tenant Community OS with real-life payments**: eight sourced research passes (accounts and passkeys, Stripe Connect, crowdfunding law, POS and stablecoin law, revenue benchmarks, multi-tenant Nostr and org infrastructure, two internal digests) mapped onto the August kickoffs. Verdicts per pillar: two custody tiers (passkey plus guardians on a Safe stack, assisted node-held signer), thirdweb bridge-now/re-key-later, roles-onchain identities-off membership, one Circles group per town with earned Münzen as points, crowdfunding as sales with donations by Sparkasse/Volksbank partner (AMLR makes donation platforms obliged entities from 2027-07-10), Stripe direct charges kept, **Ortis Kasse with a certified TSE from day one** (EURe is e-money, so a cashless Kasse still has Kassenfunktion), payer privacy as sub-account and stealth Safes (unlinkable, not confidential). Revenue ranking (Amt licence at about €1 per inhabitant, local sponsor, org subscriptions, settlement fees), a tenant P&L, grants with dates (Prototype Fund closes 2026-11-30), gated roadmap, verification list, 14 decisions for Max | Draft for review, 2026-09-25 |
