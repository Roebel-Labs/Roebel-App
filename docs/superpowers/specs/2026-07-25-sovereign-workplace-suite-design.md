...
| **P3 — Agent Runtime v0** ★ *(identity/delegation/audit slice **✅ implemented**, `apps/roebel-id`)* | agent principal: `actor_type`→**`act` delegation** + **client-credentials grant** + **Zodiac-scoped Safe budget** + MCP tools + audit; Mecky as the reference agent. Shipped: `id_agents` registry, `client_credentials` JWT minting, RFC 8693 `act` delegation + `roebel:scopes`, audit trail (`id_agent_audit`), and the kill switch (`enabled=false` → no token). Remaining: **Zodiac budget enforcement (P3b)** — `budget_ref` is stored/surfaced but not enforced — and **MCP tool runtime (P3c)** | P1, Safe/Zodiac, MCP | an agent, on behalf of an org, drafts a doc + proposes a bounded treasury payment, fully audited, within limits |
