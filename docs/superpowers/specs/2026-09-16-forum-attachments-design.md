...
| Any-file picking | **OTA only, no new build (Max, 2026-09-16).** No native picker module is added. Files are picked through a `FilePickerSheet`: a modal `react-native-webview` (already in the binary for mini-apps) hosting an `<input type="file">`; the page reads the file with `FileReader.readAsDataURL` and posts `{ name, type, size, base64 }` to the app, which uploads the bytes. Limit 15 MB on this path. Images use `expo-image-picker`. |
...
- `uploadForumFileFromBase64(base64, mime, name, size)`: same upload path without the FileSystem read (the WebView picker already delivers base64).
- `components/forum/FilePickerSheet.tsx`: modal WebView with an `<input type="file" accept="…">`, `onMessage` → `onPicked({ name, mime, size, base64 })`; `lib/file-picker-html.ts` builds the HTML (pure, jest-covered for the accept list and the 15 MB guard message).
