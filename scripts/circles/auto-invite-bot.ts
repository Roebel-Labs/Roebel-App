...
async function main() {
  ...
  console.log(`to trust: ${toTrust.length} | eligible: ${eligible.length} | skipped (fail gate): ${skipped.length}`, skipped);
  if (!eligible.length) {
    console.log("nothing eligible ✓");
    return;
  }

  // 4) Trust the eligible members in batches.
