...
export default function OrgProductsScreen() {
  ...
  const loadListings = useCallback(async () => {
    ...
    try {
      const data = await fetchOrgListings(activeAccount.id, 'product');
      setListings(data);
    } catch (error) {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>Produkte</Text>
      ...
      >
        ...
        {activeListings.length > 0 && (
          <View style={styles.section}>
            <Text style={[styles.sectionTitle, { color: colors.textPrimary }]}>
              Aktive Produkte ({activeListings.length})
            </Text>
        ...
        {listings.length === 0 && !loading && (
          <View style={[styles.emptyState, { backgroundColor: colors.surface }]}>
            ...
            <Text style={[styles.emptyTitle, { color: colors.textPrimary }]}>Keine Produkte</Text>
            <Text style={[styles.emptyText, { color: colors.textTertiary }]}>
              Füge dein erstes Produkt hinzu, damit es im Marktplatz erscheint.
            </Text>
          </View>
        )}
