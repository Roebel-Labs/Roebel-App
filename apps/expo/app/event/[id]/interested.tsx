...
import { listInterestedUsers, type InterestedUser } from '@/lib/supabase-interests';
...
export default function EventInterestedList() {
  ...
  useEffect(() => {
    if (!id) return;
    listInterestedUsers(id).then((data) => {
      setUsers(data);
      setLoading(false);
    });
  }, [id]);

  const handleOpenProfile = (user: InterestedUser) => {
    if (!user.username) return;
    router.push({ pathname: '/user/[username]', params: { username: user.username } });
  };

  const renderUser = ({ item, index }: { item: InterestedUser; index: number }) => {
    const canOpen = !!item.username;
    const displayName = item.username || 'Unbekannt';
    return (
      <Pressable
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.textPrimary }]}>Interessiert</Text>
