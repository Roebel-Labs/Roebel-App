...
export default function SideSelectionGroup({ label, required, sides, value, onChange }: Props) {
      ...
      <View style={{ marginTop: 8 }}>
        {sides.map((side, idx) => {
          ...
          return (
            ...
            >
              <View style={{ flex: 1 }}>
                <Text style={[styles.name, { color: colors.textPrimary }]}>{side.name}</Text>
                {!!side.description && (
                  <Text style={[styles.desc, { color: colors.textSecondary }]}>{side.description}</Text>
                )}
                {Number(side.price_delta) > 0 && (
                  <Text style={[styles.delta, { color: colors.textSecondary }]}>
                    + €{Number(side.price_delta).toFixed(2)}
                  </Text>
                )}
              </View>
              <View
...
const styles = StyleSheet.create({
  ...
  desc: { fontSize: 13, fontFamily: 'Inter-Regular', marginTop: 2 },
  delta: { fontSize: 13, fontFamily: 'Inter-Regular', marginTop: 2 },
