...
var mockUserData = /* @__PURE__ */ new Map();
var announced = false;
...
function mockDispatch(method, params) {
  switch (method) {
    ...
    case "data.list": {
      ...
      const store = getMockConfig().data ?? {};
      const items = Object.keys(store).filter((k) => k.startsWith(prefix)).sort().map((k) => ({ key: k, value: store[k] }));
...
var ClientBridge = class {
  ...
  handshake(sdkVersion) {
    ...
    return new Promise((resolve) => {
      let retry;
      const stopRetry = () => {
        if (retry !== void 0) clearInterval(retry);
      };
