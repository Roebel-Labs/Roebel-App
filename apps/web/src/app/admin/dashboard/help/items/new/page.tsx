...
import { createItem } from "@/app/actions/help-hub"
import { createClient } from "@/lib/supabase/client"
import { ArrowLeft, Save, Eye, Plus, X, ChevronUp, ChevronDown } from "lucide-react"
...
export default function NewItemPage() {
  ...
  const handleSubmit = async (e: React.FormEvent, publish: boolean) => {
    ...
    const loadingToast = toast.loading(
      publish ? "Artikel wird veröffentlicht..." : "Entwurf wird gespeichert..."
    )

    const result = await createItem(submitData)

    if (result.success) {
      toast.success(publish ? "Artikel veröffentlicht" : "Entwurf gespeichert", {
        id: loadingToast,
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-3xl font-medium text-foreground">Neuer Hilfe-Artikel</h1>
          <p className="text-muted-foreground mt-1">Erstellen Sie einen neuen Hilfe-Artikel</p>
        </div>
      </div>
