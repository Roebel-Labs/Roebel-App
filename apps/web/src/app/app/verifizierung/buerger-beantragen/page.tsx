...
import { getBlockscoutTxUrl, getBaseScanTxUrl } from "@/lib/blockscout";
import {
  deriveEncryptionKey,
  deriveSessionKey,
  encryptEvidence,
...
export default function RequestCitizenNFT() {
  const account = useActiveAccount();
  const { isCitizen, isLoading: statusLoading } = useVerificationStatus();
  ...
  // Check eligibility
  const isEligible = account && !isCitizen && !statusLoading;
  ...
  const handleUploadToIrys = async () => {
    ...
    try {
      console.log("🔐 Starting encrypted evidence upload (V2)...");
      ...
      // Step 3: Encrypt personal data with random data key
      console.log("🔒 Encrypting personal data with V2 encryption...");
      const encryptedBlob = encryptEvidence(personalData, dataKey);
      ...
      const irysReceipt = await uploadToIrys(account, jsonString, [
        ...
        { name: "Type", value: "CitizenAttestation" },
        { name: "Encrypted", value: "true" },
        { name: "EncryptionVersion", value: "2" }, // V2: wallet-based encryption
    ...
    try {
      ...
      sendTransaction(transaction, {
        onSuccess: async (result) => {
          console.log("✅ [Bürger] Transaction successful:", {
            transactionHash: result.transactionHash,
          });
          console.log("🔗 [Bürger] View on Blockscout:", getBlockscoutTxUrl(result.transactionHash));
          console.log("🔗 [Bürger] View on BaseScan:", getBaseScanTxUrl(result.transactionHash));

          setCreationTxHash(result.transactionHash);

          try {
            // Wait for transaction receipt
            console.log("⏳ [Bürger] Waiting for receipt...");
            const receipt = await waitForReceipt({
              client,
              chain: citizenNFTContract.chain,
              transactionHash: result.transactionHash,
            });
            ...
            // The event signature: AttestationRequestCreated(uint256 indexed requestId, address indexed target, string evidenceURI)
            // Topic0 is the event signature hash, Topic1 is requestId (indexed), Topic2 is target (indexed)
            const event = receipt.logs.find(log =>
              log.address.toLowerCase() === citizenNFTContract.address.toLowerCase() &&
              log.topics.length >= 2  // Has requestId in topics[1]
            );

            if (event && event.topics[1]) {
              // RequestID is the second topic (first indexed parameter)
            ...
            setRequestId("pending"); // Fallback
          }

          setCurrentStep("success");
        },
        onError: (error) => {
          console.error("❌ [Bürger] Transaction failed:", {
          ...
          alert(`${de.errors.transactionFailed}: ${error.message || "Unbekannter Fehler"}`);
        },
      });
    } catch (error) {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        <Link href="/app/verifizierung" className="text-muted-foreground hover:text-foreground transition-colors">
          ← Zurück zur Verifizierung
        </Link>
      </div>

          <div className="mb-8">
            <h1 className="text-3xl font-medium text-foreground">{de.verification.requestCitizenNFT}</h1>
            <p className="text-muted-foreground mt-1">Werde ein verifiziertes Mitglied der Community</p>
          </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-muted-foreground mb-6">
                    Prüfe, ob du berechtigt bist, einen Bürger-Pass Antrag zu stellen
                  </p>
                </div>
                ...
                {!account ? (
                  ...
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                    ...
                    <p className="text-sm text-red-700">
                      Du besitzt bereits einen Bürger-Pass und musst keinen neuen Antrag stellen.
                    </p>
                  </div>
                ) : (
                  <>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder={de.verification.reasonPlaceholder}
                      rows={4}
                      className="w-full px-4 py-3 border border-border rounded-lg focus:ring-2 focus:ring-black focus:border-transparent"
                      required
                    />
              ...
              <div className="space-y-6">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-sm text-blue-800">
                    Nach der Erstellung wartet dein Antrag auf <strong>1 Bescheiniger + 1 Bürger</strong> Unterschrift.
                  </p>
                </div>
              ...
              <div className="space-y-6">
                ...
                {creationTxHash && (
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-6">
                    ...
                    <div className="space-y-3">
                      <a
                        href={getBlockscoutTxUrl(creationTxHash)}
                        target="_blank"
                        rel="noopener noreferrer"
                      ...
                      <a
                        href={getBaseScanTxUrl(creationTxHash)}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="block text-center text-sm text-blue-700 hover:text-blue-900 underline"
                      >
                        View on BaseScan (alternative)
                      </a>
                    </div>
                  </div>
                )}
                  ...
                  <>
                    ...
                    <div className="bg-blue-50 border border-blue-200 rounded-xl p-6">
                      ...
                      <p className="text-sm text-blue-800 mb-4 text-center">
                        Zeige diesen QR-Code einem Bescheiniger oder Bürger, damit sie deinen Antrag direkt öffnen können
                      </p>

                      {/* QR Code */}
                      <div className="bg-card p-6 rounded-lg flex justify-center mb-4">
                        <QRCodeSVG
                          value={`https://roebel.app/app/verifizierung/nachweis/${requestId}?contract=citizen`}
                          size={200}
                          level="H"
                        />
                      </div>
                      ...
                      <button
                        onClick={() => {
                          const link = `https://roebel.app/app/verifizierung/nachweis/${requestId}?contract=citizen`;
                      ...
                      <div className="text-center">
                        <Link
                          href={`/app/verifizierung/nachweis/${requestId}?contract=citizen`}
                ...
                <div className="bg-muted border border-border rounded-lg p-6">
                  ...
                  <ol className="space-y-2 text-sm text-foreground">
                    <li>1. Zeige den QR-Code einem Bescheiniger oder Bürger</li>
                    <li>2. Sie können deinen Antrag prüfen und unterschreiben</li>
                    <li>3. Nach 1 Bescheiniger + 1 Bürger Unterschrift wird dein Pass vergeben</li>
                  </ol>
                </div>
