...
export function computeCanvasScreens(
  ...
  if (!streaming) {
    if (base.length === 0 && baseHtml) {
      ...
      return [{ name: "app", title: "App", phase: "idle", doc: baseHtml }];
    }
    return base.map((s) => ({ name: s.name, title: s.title, phase: "idle", doc: baseDoc(s.name) }));
  }
  ...
  for (const s of incoming) {
    ...
    if (s.outer === null) {
      ...
      out.push({
        ...
        title: s.title,
        phase: "writing",
        doc: prev ? baseDoc(s.name) : null,
      });
    } else if (prev?.outer === s.outer) {
      out.push({ name: s.name, title: s.title, phase: "done-same", doc: baseDoc(s.name) });
    } else {
      out.push({
        name: s.name,
        title: s.title,
        phase: "done-changed",
        doc: buildPartialScreenDoc(stream, s.outer) ?? (prev ? baseDoc(s.name) : null),
      });
    }
  }
  ...
  for (const s of base) {
    if (!seen.has(s.name)) {
      out.push({ name: s.name, title: s.title, phase: "pending", doc: baseDoc(s.name) });
    }
  }

  // First generation, before any section appears: one shimmering placeholder.
  if (out.length === 0) {
    out.push({ name: "app", title: "App", phase: "writing", doc: null });
  }
  return out;
}
