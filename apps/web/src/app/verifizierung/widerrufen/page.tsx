...
export default function RevokeMembershipPage() {
  ...
  // Pre-flight: check target holds the NFT being revoked. Skip in self mode
  // (we already know from useVerificationStatus). Debounced via input value.
  useEffect(() => {
    if (mode === "self") {
  ...
  const shareUrl =
    requestId && requestId !== "pending"
      ? `${origin}/verifizierung/nachweis/${requestId}?contract=${contractType}`
      : "";
...
function SuccessStep({
  ...
  useEffect(() => {
    const measure = () => {
      ...
      // padding inside white tile is 16px each side
  ...
  // Avoid `resolvedTheme` lint-unused while keeping the hook for future theming
  void resolvedTheme;
  ...
  const nativeShare = async () => {
    ...
    try {
      ...
      // user-cancel raises AbortError — silent is correct
