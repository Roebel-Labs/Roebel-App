// Citizen node for the citizen-verification graph (ported from apps/web, adapted to
// @xyflow/react). Renders a navy "Bürger:in" card with pending / revoked / founder states.
import { memo } from "react";
import { Handle, Position, type NodeProps } from "@xyflow/react";
import type { CgStatus } from "../../lib/citizen-graph";

function CitizenNode({ data }: NodeProps) {
  ...
  const activeClass = d.isFounder
    ? "bg-gradient-to-br from-[#00498B] to-[#003468] border-[#3b82f6]"
    : "bg-gradient-to-br from-[#0a5aa8] to-[#00498B] border-[#5a9be0]";
    ...
    <div className="relative">
      ...
      <div className={wrapperClass}>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="font-medium">Bürger:in</span>
        </div>

        <div className={`text-xs opacity-90 font-mono ${status === "revoked" ? "line-through" : ""}`}>{shortAddress}</div>

        {status === "pending" && (
          <div className="mt-2 text-xs bg-blue-200/60 text-blue-900 px-2 py-1 rounded">Antrag offen</div>
        )}
