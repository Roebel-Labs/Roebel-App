...
/// @title AttesterNFTv2
/// @notice Soulbound NFT for Roebel culture-committee members, with **scale-aware,
///         percentage-based** signature thresholds (see ThresholdBands).
///
/// Differences vs AttesterNFT (v1):
///  - `approvalBand` / `rejectionBand` replace the flat `requiredSignatures` /
///    `requiredRejections`. The required signer count scales with the live
///    `attesterCount`, bounded by floor/cap.
///  - Each request **snapshots** its required approval/rejection counts at creation
///    time, so a later band change or set-size change can't move the goalposts mid-request.
///  - `attesterCount` is exposed so CitizenNFTv2 can use it as a denominator.
contract AttesterNFTv2 is ERC721, Ownable {
    ...
    /// @notice Live number of attesters (denominator for percentage bands).
    ...
    /// @notice Threshold for minting/revoking an attester, and for vetoing a request.
    ...
    function createRevocationRequest(address target, string calldata evidenceURI) external {
        ...
        _createRequest(target, RequestType.Revocation, evidenceURI);
    }
    ...
    function _createRequest(address target, RequestType reqType, string calldata evidenceURI) internal returns (uint256) {
        ...
        // Snapshot required counts from the current attester set size.
    ...
    function _executeRequest(uint256 requestId) internal {
        Request storage req = requests[requestId];
        require(req.status == RequestStatus.Pending, "Request not pending");
        require(req.signatureCount >= req.requiredApprovals, "Not enough signatures");
