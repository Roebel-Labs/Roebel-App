...
contract PasskeySafeSponsoredTest is PasskeySafeBase {
    ...
    function _writeVectorJson(
        ...
        vm.serializeString(w, "_note", "Inputs: challenge = userOp.safeOpHash; authenticatorData = sha256('roebel.app') ++ 0x05 (UP|UV) ++ uint32 signCount 0; clientDataJSON = bytes of clientDataJSONHex; (r,s) from P-256 key passkeyPrivateKey over signingDigest = sha256(authenticatorData ++ sha256(clientDataJSON)), s normalized to low-s. clientDataFields = clientDataJSON minus the leading '{\"type\":\"webauthn.get\",\"challenge\":\"<b64url>\",' and the trailing '}'. webAuthnSignature = abi.encode(bytes authenticatorData, string clientDataFields, uint256 r, uint256 s) (verified on the real SafeWebAuthnSignerFactory.isValidSignatureForSigner). safeSignature = bytes32(owner) ++ bytes32(65) ++ 0x00 ++ uint256(len) ++ webAuthnSignature with owner = SafeWebAuthnSharedSigner. userOpSignature = uint48 validAfter ++ uint48 validUntil ++ safeSignature.");
